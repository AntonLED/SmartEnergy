...
boolean startServer_4Pr = true;//  флаг для единичного запуска setup сервера 3-ого приоритета
boolean startClient_5Pr = true;
boolean startClient_Bat = true;
boolean startClient_Gen = true;
boolean Server_4Pr_flag; // флаг на добро для поднятия сервера
...
String GAS_ID = "AKfycbxGh1pUacu2wJLGJ7WhUYYJ8V9bEaT7694TEw2obx2U1PBXGl0";
...
void Server_4Pr_Setup() {
  WiFi.mode(WIFI_AP);
  WiFi.softAP("MG4p", "12345678904p");
  server.begin();
...
void Cloud_Send(String fro, int cur, int vol, int cap) {
  ...
  client_secure.print(String("GET ") + "/macros/s/AKfycbxGh1pUacu2wJLGJ7WhUYYJ8V9bEaT7694TEw2obx2U1PBXGl0" + url + " HTTP/1.1\r\n" +
                      "Host: " + host + "\r\n" +
...
void sensor_update() {
  ...
  data_4Pr[0] = id_4Pr;
  data_4Pr[1] = current_mA4; 
  data_4Pr[2] = loadvoltage4; 
  data_4Pr[3] = power_mW4;
}
...
//режим сервера для транслирования информации
  ...
  if (Server_4Pr_flag == true) {
    if (startServer_4Pr == false) {
      Server_4Pr_Setup();
      startServer_4Pr = true;
      Serial.println("S4S");
    }
    for (int x=0; x<6; x++){
    sensor_update();
    delay(500);
          while (true) {
            ...
            if (client && client.connected()) { // если есть клиент с непрочитанными данными
              client.write('D');    // контрольный символ отправки (кто отпраляет)
              for (int i = 0; i < PARSE_AMOUNT; i++) { // отправка массива данных
                client.print(data_4Pr[i]);
                client.print(" ");
                Serial.print(data_4Pr[i]);
                Serial.print(" ");
              }
              client.write('E'); // контрольный символ отправки (кому передать эстафету)
              break;
            }
            delay(1);
          }
