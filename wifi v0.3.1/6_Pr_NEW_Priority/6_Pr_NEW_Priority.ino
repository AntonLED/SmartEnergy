...
boolean startServer_Bat = true; 
boolean startClient_Gen = true; 
boolean Server_Bat_flag; // флаг на добро для понятия сервера 
...
void Server_Bat_Setup(){
  WiFi.mode(WIFI_AP); 
  WiFi.softAP("MGBat", "1234567890Bat"); 
  server.begin(); 
...
void Cloud_Send(String fro, int cur, int vol, int cap) {
  ...
  client_secure.print(String("GET ") + "/macros/s/AKfycbxP_gKwBsyQcIPcaxsKb5C_w4tG2TT9Ts1Mw2BZmYOzJ3WQNarO" + url + " HTTP/1.1\r\n" +
         "Host: " + host + "\r\n" +
...
//eeprom
void sensor_update() {
  shuntvoltage6 = inaBat.getShuntVoltage_mV();
  busvoltage6 = inaBat.getBusVoltage_V();
  current_mA6 = inaBat.getCurrent_mA();
  power_mW6 = inaBat.getPower_mW();
  loadvoltage6 = busvoltage6+ (shuntvoltage6 / 1000);
  
  data_Bat[0] = id_6Pr;
  data_Bat[1] = current_mA6; 
  data_Bat[2] = loadvoltage6; 
  data_Bat[3] = power_mW6;
}
...
sensor_update();// функция обычной работы датчика
  ...
  Serial.println(" ");
     if(Server_Bat_flag == true){
       if(startServer_Bat == false){
        Server_Bat_Setup(); 
        startServer_Bat = true; 
        Serial.println("S6S");
       }
       for(int x=0; x<6; x++){
          sensor_update(); 
           ...
           while(true){
             WiFiClient client = server.available(); // объект клиент
              if(client && client.connected()){   // если есть клиент с непрочитанными данными 
                    client.write('F');    // контрольный символ отправки (кто отпраляет)  
                    for(int i = 0; i < PARSE_AMOUNT; i++){  // отправка массива данных 
                      client.print(data_Bat[i]);
                      client.print(" "); 
                      Serial.print(data_Bat[i]); 
                      Serial.print(" "); 
                    }
                    client.write('G'); // контрольный символ отправки (кому передать эстафету)
                    break;
              }  
              delay(1); 
           }
