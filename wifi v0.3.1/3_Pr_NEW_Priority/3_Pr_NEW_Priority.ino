...
#define PARSE_AMOUNT_GEN 7  // число значений в массиве, который хотим получить
#define RELAY_PIN 12  // число значений в массиве, который хотим получить
...
boolean startServer_3Pr = true;//  флаг для единичного запуска setup сервера 3-ого приоритета
...
boolean Server_3Pr_flag; // флаг на добро для понятия сервера
...
String GAS_ID = "AKfycbzhY25bX21xMJBjRnIBKkrPkyvZK1vxhSlq40GGVh3YmetP0Sqw";
...
void Server_3Pr_Setup() {
  WiFi.mode(WIFI_AP);
  WiFi.softAP("MG3p", "12345678903p");
  server.begin();
...
void sensor_update() {
  ...
  data_3Pr[0] = id_3Pr;
  data_3Pr[1] = current_mA3; 
  data_3Pr[2] = loadvoltage3; 
  data_3Pr[3] = power_mW3;
}
...
//режим сервера для транслирования информации
  ...
  if (Server_3Pr_flag == true) {
    if (startServer_3Pr == false) {
      Server_3Pr_Setup();
      startServer_3Pr = true;
      Serial.println("S3S");
    }
    for (int x=0; x<6; x++){
    sensor_update();
    delay(500);
          while (true) {
            ...
            if (client && client.connected()) { // если есть клиент с непрочитанными данными
              client.write('C');    // контрольный символ отправки (кто отпраляет)
              for (int i = 0; i < PARSE_AMOUNT; i++) { // отправка массива данных
                client.print(data_3Pr[i]);
                client.print(" ");
                Serial.print(data_3Pr[i]);
                Serial.print(" ");
              }
              client.write('D'); // контрольный символ отправки (кому передать эстафету)
              break;
            }
            delay(1);
          }
     ...
     delay(500);
    Server_3Pr_flag = false;
    startClient_4Pr = false;
    Serial.println();
  }
  delay(100);
//клиент для подключения ко 4-ому приоритету
...
//клиент для подключения ко 5-ому приоритету
...
//клиент для подключения ко 6-ому приоритету
...
//клиент для подключения ко 7-ому приоритету
  ...
  if (recievedFlag) {
    ...
    if (allPower >= 1810 && allPower < 4330) {
      digitalWrite(RELAY_PIN, LOW);
    }
    else if (allPower > 5960) {
      digitalWrite(RELAY_PIN, LOW);
    }
    else {
