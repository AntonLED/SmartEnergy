...
#define PARSE_AMOUNT_GEN 7 // число значений в массиве, который хотим получить от генераторов
...
boolean startServer_2Pr = true;//  флаг для единичного запуска setup сервера 2-ого приоритета
boolean startClient_3Pr = true; // флаг для единичного запуска setup клиента для подключения к серверу 3-ого приоритета
boolean startClient_4Pr = true;  //  флаг для единичного запуска setup клиента для подключения к серверу 4-ого приоритета
boolean startClient_5Pr = true;
boolean startClient_Bat = true;
boolean startClient_Gen = true;
boolean Server_2Pr_flag; // флаг на добро для понятия сервера
...
String GAS_ID = "AKfycbyGjkrZJdYt8bv4R4SZC2nPtGqI8kXOWeCCOgxp_aLdHFG7a5ac";
...
void Server_2Pr_Setup() {
  WiFi.mode(WIFI_AP);
  WiFi.softAP("MG2p", "12345678902p");
  server.begin();
...
void parsing_1Pr() {
  while (client.available() > 0) {
    ...
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != ' ' && incomingByte != 'B') {  // если это не пробел И не конец строки
    ...
    if (incomingByte == 'B') {
      ...
      Server_2Pr_flag = true;                      // добро на становление сервером
    }
  }
}
void parsing_3Pr() {
  while (client.available() > 0) {
    ...
    //Serial.println(incomingByte);
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != ' ' && incomingByte != 'D') {  // если это не пробел И не конец строки
        string_convert += incomingByte;       // складываем в строку
      }
      else {
...
void parsing_4Pr() {
  while (client.available() > 0) {
    ...
    //Serial.println(incomingByte);
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != ' ' && incomingByte != 'E') {  // если это не пробел И не конец строки
        string_convert += incomingByte;       // складываем в строку
      }
      else {
...
void parsing_5Pr() {
  while (client.available() > 0) {
    ...
    //Serial.println(incomingByte);
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != ' ' && incomingByte != 'F') {  // если это не пробел И не конец строки
        string_convert += incomingByte;       // складываем в строку
      }
      else {
...
void parsing_Bat() {
  while (client.available() > 0) {
    ...
    //Serial.println(incomingByte);
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != ' ' && incomingByte != 'G') {  // если это не пробел И не конец строки
        string_convert += incomingByte;       // складываем в строку
      }
      else {
...
void parsing_Gen() {
  while (client.available() > 0) {
    ...
    //Serial.println(incomingByte);
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != ' ' && incomingByte != 'A') {  // если это не пробел И не конец строки
        string_convert += incomingByte;       // складываем в строку
      }
      else {
...
void sensor_update() {
  ...
  data_2Pr[0] = id_2Pr;
  data_2Pr[1] = current_mA2; 
  data_2Pr[2] = loadvoltage2; 
  data_2Pr[3] = power_mW2;
}
...
sensor_update();// функция обычной работы датчика
  ...
  if (Server_2Pr_flag == true) {
    if (startServer_2Pr == false) {
      Server_2Pr_Setup();
      startServer_2Pr = true;
      Serial.println("S2S");
    }
    for (int x=0; x<6; x++){
    sensor_update();
    delay(500);
          while (true) {
            ...
            if (client && client.connected()) { // если есть клиент с непрочитанными данными
              client.write('B');    // контрольный символ отправки (кто отпраляет)
              for (int i = 0; i < PARSE_AMOUNT; i++) { // отправка массива данных
                client.print(data_2Pr[i]);
                client.print(" ");
                Serial.print(data_2Pr[i]);
                Serial.print(" ");
              }
              client.write('C'); // контрольный символ отправки (кому передать эстафету)
              break;
            }
            delay(1);
          }
