...
int dataTo[8] = {1, 22, 222, 2222, 33, 333, 3333, 2}; // для хранения отсылаемых данных 
int dataSystem[7][6]; // массив-база данных

byte myPriority = 7; // для хранения приоритета данного модуля
...
void sensorUpdate(){
  ...
  dataTo[1] = current_mA1; //сила тока на 1-ом генераторе
  dataTo[2] = loadvoltage1; //напряжение на 1-ом генераторе
  dataTo[3] = power_mW1; // мощность на 1-ом генераторе
  dataTo[4] = current_mA2; //сила тока на 1-ом генераторе
  dataTo[5] = loadvoltage2; //напряжение на 1-ом генераторе
  dataTo[6] = power_mW2; // мощность на 1-ом генераторе
...
int Recieve_Loop(){
  ...
  while(true){
    if(radio.available()){
      ...
      if(dataFrom[0] != 0 && dataFrom[4] == myPriority){
...
void setup() {
  ...
  sensor1.begin(); // активация датчиков
  sensor2.begin(); // активация датчиков
